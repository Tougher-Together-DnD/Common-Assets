...
on("chat:message", function(msg) {
	...
	var style = "width: 189px; border: 1px solid black; background-color: #ffffff; padding: 5px; font-size: 16px;"

	// width: 189px; border: 1px solid black; background-color: #ffffff; padding: 5px; font-size: 12px;
	// "padding:2px;border: 1px solid black;background: #eeffee; color: #8B4513; font-size: 80%;border-radius: 4px;  box-shadow: 1px 1px 1px #707070; width: 100px;
	// style="text-align:center; border: 1px solid black; margin: 1px; padding: 2px; background-color:#7E2D40;border-radius: 4px;  box-shadow: 1px 1px 1px #707070; width: 100px;
    ...
    if(msg.type == "api" && msg.content.indexOf("!range") != -1)
    {
        ...
        // DnD 4.0 Distance Calculation
            ...
            var zDist = Math.abs((token1.get("bar3_value")-token2.get("bar3_value"))/curScale);//gridSize;
            var t1Height = token1.get("height");// to use for token size variation in the fiture
			if (zDist >= 1)
				{
				zDist = zDist - 0;// based on medium size tokens reduce effective height difference by 1 unit as token is a cube
				}
			...
			var distSQ = dist;
			dist = dist * curScale;
			var finalMessage = ("Distance from "+name1+" to Target = (" +distSQ+") "+dist+" "+curUnit);
        ...
        // Hex(H) Distance Calculation - Including distance between "Is Drawing" objects to 3 decimal places
			...
			dist = 1.5 * Math.min(lDist, tDist) + Math.abs(lDist - tDist);
			dist = Math.round(dist * curScale * 1000) / 1000; // Decimal point accuracy limit
			// var finalMessage = ("Distance from "+name1+" to Target = " +dist+" "+curUnit+" left "+lDist+" top "+tDist);
			var finalMessage = ("Distance from "+name1+" to Target = " +dist+" "+curUnit);
			sendChat("RangeFinder",'<div style="' + style +'">'+finalMessage+'</div>');
            }
        }
